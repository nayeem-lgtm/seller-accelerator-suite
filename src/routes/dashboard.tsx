      ...
      <div className="mx-auto max-w-6xl px-4 py-10 lg:px-8 space-y-6">
        ...
        <div className="rounded-2xl border border-border bg-white p-6 shadow-sm">
          <div className="grid grid-cols-[minmax(0,1fr)_auto] items-start gap-4 sm:flex sm:items-center sm:justify-between">
            ...
            {isAdmin && (
              <Button asChild className="shrink-0 rounded-full brand-gradient text-white btn-glow">
                <Link to="/admin">
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          ...
          <SummaryCard
            ...
            value={marketplace}
            helper={hasMarketplace ? "Active plan" : "No plan selected yet"}
          />
          <SummaryCard
            icon={<CreditCard className="h-4 w-4" />}
            title="Payment"
            value={pay.label}
            helper={payment?.amount ? `${payment.currency ?? "USD"} ${Number(payment.amount).toFixed(2)}` : "Awaiting payment"}
          />
          <SummaryCard
            icon={<UserCheck className="h-4 w-4" />}
            title="Account"
            value={acct.label}
            helper={`Updated ${formatDate(profile?.updated_at)}`}
          />
        </div>

        {/* Onboarding tracker + Next action */}
            ...
            <ul className="mt-6 space-y-3">
              {STEPS.map((s, i) => (
                <li key={s} className="flex items-center gap-3 text-sm">
                  ...
                  <span className={i === stepIndex ? "font-semibold text-primary" : i < stepIndex ? "font-medium" : "text-muted-foreground"}>
                    {s}
                  </span>
                  {i === stepIndex && (
        ...
        {/* Service + Payment + Account detail */}
        <div className="grid gap-6 md:grid-cols-3">
          ...
          >
            ...
            <p className="mt-3 text-xs text-muted-foreground">
              Your account is {acct.label.toLowerCase()} and onboarding is in progress.
            </p>
          </DetailCard>
        </div>
          ...
          <div className="rounded-2xl border border-border bg-white p-6 shadow-sm">
            ...
            {activity.length === 0 ? (
              <p className="mt-3 text-sm text-muted-foreground">
                No recent activity yet. Complete your next step to begin onboarding.
              </p>
            ) : (
...
function DetailCard({
  ...
  return (
    <div className="rounded-2xl border border-border bg-white p-6 shadow-sm transition hover:shadow-md">
